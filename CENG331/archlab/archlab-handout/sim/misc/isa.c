...
instr_t instruction_set[] = 
{
    ...
    {"jmpq",   HPACK(I_JMPQ, F_NONE), 2, R_ARG, 1, 1, NO_ARG, 0, 0 },
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVQ || hi0 == I_ALU || hi0 == I_PUSHQ ||
	 ...
	 hi0 == I_MRMOVQ || hi0 == I_IADDQ || hi0 == I_JMPQ);
	...
	s->pc = ftpc;
	break;
    /* Let's try to simulate jmpq here! Sure lots of copy-paste... (DS) */
    case I_JMPQ:
    if (!ok1) {
        if (error_file)
            fprintf(error_file,
    ...
    /* Get the register value and set the pc to it, nothing simpler! */
